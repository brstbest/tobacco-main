        ...
        <template v-for="(item, index) in recordColumns.slice(0, 9)">
          ...
          :width="item.width">
            <template v-if="item.key === 'operate'" v-slot="scope">
              <el-button size="mini" type="primary" plain @click="detailClick(scope.row)">详细信息</el-button>
...
<script>
  ...
  import BakeDetail from './components/Detail'
    ...
    data() {
      return {
        ...
        recordColumns: [
          ...
          { key: 'operate', label: '操作', width: '250' },
